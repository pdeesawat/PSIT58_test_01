...
for i in range(5):
    ...
    trace.append(go.Scatter(x = year_x, y = affected_z, name = type_z[i], mode = 'markers',
    marker=dict(color = [fill_colors[i] for k in affected_z], size = [size_fill[i] for k in affected_z])))
    
data = trace

"""Part of code that adjust layout of graph"""
layout = go.Layout(title='Total Affected',
                   showlegend=True,
                   height=600,
                   width=600,
...
plot_url = py.plot(fig, filename='Total_Affect_in_Indonesia')
