...
for i in range(5):
    ...
    trace.append(go.Scatter(x = year_x, y = affected_z, name = type_z[i],
                            line = dict(color = fill_colors[i], width = 1.5)))

data = trace

"""Set Style Layout"""
layout = go.Layout(title = 'Total Affected', width = 800, height = 482, 
...
plot_url = py.plot(fig, filename='Total_Affected_in_Cambodian')