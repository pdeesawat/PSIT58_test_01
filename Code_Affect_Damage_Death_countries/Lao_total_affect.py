...
for i in range(5):
    ...
    trace.append(go.Bar(x = year_x, y = affected_z, name = type_z[i],
                        marker=dict(color = [fill_colors[i] for k in affected_z])))
...
layout = go.Layout(barmode = 'group', 
                   ...
                   yaxis = dict(title = 'Total Affected',
                                titlefont = dict(size = 16, color = '#eb0056'),
                                tickfont = dict(size = 14, color = '#eb0056')))
                                
"""Part of plot graph in plotly"""
fig = go.Figure(data=data, layout=layout)
plot_url = py.plot(fig, filename='Total_Affected_in_Laos')