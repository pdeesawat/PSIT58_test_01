...
for i in range(5):
    ...
    types_y = []
    for j in listdata:
        if j[0] == 'Singapore' and j[2] == type_z[i]:
            year_x.append(int(j[1]))
            affected_z.append(int(j[3]))
            types_y.append(type_z[i])
    if year_x != []:
        trace_list[num] = go.Scatter3d(x = year_x, y = types_y, z = affected_z,
        mode = 'markers', marker = dict(size=12, line = dict(color = fill_colors[num], width = wide),
        opacity = opac))
...
layout = dict(
    ...
    scene = dict(xaxis = dict(title = 'Year'), yaxis = dict(title = 'Disaster'), zaxis = dict(title = 'Total Affected')))
    
fig = go.Figure(data = last, layout = layout)
plot_url = py.plot(fig, filename = 'Total Affected Singapore')
