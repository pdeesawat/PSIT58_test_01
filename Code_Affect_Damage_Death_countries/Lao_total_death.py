...
for i in range(5):
    ...
    trace.append(go.Bar(x = year_x, y = death_z, name = type_z[i],
                        marker=dict(color = [fill_colors[i] for k in death_z])))
...
layout = go.Layout(barmode = 'group', 
                   ...
                   yaxis = dict(title = 'Total Death',
                                titlefont = dict(size = 16, color = '#ff2323'),
                                tickfont = dict(size = 14, color = '#ff2323')))
                                
"""Part of plot graph in plotly"""
fig = go.Figure(data=data, layout=layout)
plot_url = py.plot(fig, filename='Total_Death_in_Laos')