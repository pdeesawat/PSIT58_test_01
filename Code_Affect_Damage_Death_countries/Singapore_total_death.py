...
for i in range(5):
    ...
    if year_x != []:
        trace_list[num] = go.Scatter3d(x = year_x, y = types_y, z = death_z,
...
layout = dict(
    title = 'Total Death',
    scene = dict(xaxis = dict(title = 'Year'), yaxis = dict(title = 'Disaster'), zaxis = dict(title = 'Total Death')))
    
fig = go.Figure(data = last, layout = layout)
plot_url = py.plot(fig, filename = 'Total Death Singapore')
